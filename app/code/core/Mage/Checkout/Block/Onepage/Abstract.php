...
/* */
    ...
    public function getMkAddressesHtml($type)
    {
        if ($this->isCustomerLoggedIn()) {
            $options = array();
            foreach ($this->getCustomer()->getAddresses() as $address) {
            ...
            //return $select->getHtml();
            return array(
                'new_row' => $select->getHtml(),
                'addres_user' => $address_user,
            );
        }
        return '';
    }
    ...
    {
        return array('login', 'shipping',  'billing', 'shipping_method', 'payment', 'review');
    }
